...
const Wishlist = () => {
    const { wishlistItems, removeFromWishlist, toggleWishlist, refreshWishlist } = useWishlist();
    ...
    const handleMoveToCart = async (product) => {
        try {
            if (isAuthenticated && product.wishlistItemId) {
                // Use the specialized move-to-cart API
                await wishlistService.moveToCart(product.wishlistItemId);
                
                // Add locally to cart for immediate feedback
                addToCart(product.originalData || product, 1, {
                    type: product.type,
                ...
                // Refresh both to ensure perfect sync
                refreshCart();
                refreshWishlist();
            } else {
    ...
    return (
        <div className="bg-white min-h-screen pt-32 pb-10">
            <div className="container px-4 mx-auto">
                {/* Breadcrumbs */}
                <div className="flex items-center gap-2 text-[10px] text-gray-400 uppercase mb-8">
                ...
                {/* Header */}
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4 border-b border-gray-100 pb-1">
                    <div>
                        <h1 className="text-xl font-poppins text-gray-900">My Wishlist ({wishlistItems.length})</h1>
                        {/* <p className="text-[13px] text-gray-500">You have {wishlistItems.length} items in your wishlist.</p> */}
                    </div>
                    {/* {wishlistItems.length > 0 && (
                        <button 
                            onClick={MoveAllToBag}
                            className="text-[11px] font-bold uppercase  text-[#2b5c6b] hover:text-[#1b3a4f] transition-colors underline underline-offset-4"
                        >
                            Move All To Bag
                        </button>
                    )} */}
                ...
                {wishlistItems.length === 0 ? (
                    ...
                    >
                        ...
                        <Link to="/shop" className="bg-black text-white px-12 py-4 text-[11px] font-bold  uppercase hover:bg-gray-800 transition-all rounded-sm shadow-lg shadow-black/5">
                            Shop Collections
                        </Link>
                    </motion.div>
                ) : (
                            ...
                            {wishlistItems.map((product) => (
                                ...
                                >
                                    ...
                                    <div className="relative aspect-square bg-[#fcfcfc] overflow-hidden flex items-center justify-center p-8">
                                        <Link to={product.type === 'diamond' ? `/diamond-shop/detail/${product.id}` : `/product/${product.id}`} className="block w-full h-full">
                                            <img
                                                src={product.image}
                                                alt={product.title}
                                        ...
                                        {/* Remove Action */}
                                        <button
                                            onClick={() => toggleWishlist(product.originalData || product, { type: product.type })}
                                        ...
                                        {/* <div className="absolute bottom-4 left-4 right-4">
                                            <div className="bg-white/90 backdrop-blur-sm border border-gray-100 px-3 py-1.5 flex flex-col items-center shadow-sm opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-500">
                                                <span className="text-[8px] uppercase  text-[#a67c00] font-bold">Estimated Delivery</span>
                                    ...
                                    <div className="p-5 flex flex-col flex-grow bg-white">
                                        <div className="flex-grow mb-4">
                                            <Link to={product.type === 'diamond' ? `/diamond-shop/detail/${product.id}` : `/product/${product.id}`} className="block">
                                        ...
                                        {/* <div className="flex gap-2">
                                            <button
                                                ...
                                                className="flex-1 bg-black text-white h-10 flex items-center justify-center gap-2 text-[10px] font-bold uppercase  hover:bg-gray-800 transition-colors rounded-sm"
                                            >
                                                <ShoppingBag size={14} />
                                                Add To Bag
                                            </button>
                ...
                {/* <div className="mt-20 py-12 border-t border-gray-100 grid grid-cols-2 lg:grid-cols-4 gap-8">
                    {[
                        ...
                        <div key={i} className="flex flex-col items-center text-center">
                            ...
                            <h4 className="text-[11px] font-bold uppercase  text-gray-800 mb-1">{badge.title}</h4>
